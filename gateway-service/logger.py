...
def setup_logger() -> None:
    ...
    logger.add(
        ...
        level=LOG_LEVEL,
        rotation=LOG_ROTATION,        # 每天0点轮转分割
        retention=LOG_RETENTION_DAYS,  # 保留7天
        compression="zip",             # 过期日志压缩为zip，节省树莓派存储
        encoding="utf-8",
        enqueue=True,                  # 异步写入，避免阻塞主线程（重要：适配树莓派IO性能）
    )
