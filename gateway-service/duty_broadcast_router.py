...
2. POST接口：后端调用 POST /algorithm/badge/duty-broadcasts/tts 触发播报
3. 入参校验：deviceNo非空且长度≤20，broadcastContent非空且长度≤200字
4. 联动逻辑：入参校验 → TTS可用性检查 → 设备在线检查 → 流式合成 → 流式推送

接口规范：
- 接口路径：POST /algorithm/badge/duty-broadcasts/tts
...
处理流程（严格按顺序）：
    ...
    ④ 调用PiperTTSManager.synthesize_stream获取音频流
...
使用示例：
    ...
    # POST播报请求
    # curl -X POST http://网关IP:8090/algorithm/badge/duty-broadcasts/tts \
...
@duty_broadcast_router.websocket("/ws/device/{device_no}")
...
def _build_broadcast_response(
    ...
    通过data.success区分成功/失败，对齐v3文档6.2节算法返回规范
